...
- **lemon-grove** [📁](./lemon-grove) [🌐](https://github.com/GerHobbelt/lemon-grove) -- the original Lemon parser generator, along with sibling projects, forks and assets — all in one place. The Lemon program is an [LALR(1)]  [parser generator], written in C89 by [Richard Hipp] during the late 1980s. It takes a context free grammar and converts it into a subroutine that will parse a file using that grammar. The parser generated by Lemon is both re-entrant and thread-safe. Lemon allows multiple parsers to be running simultaneously; Yacc and Bison do not.
