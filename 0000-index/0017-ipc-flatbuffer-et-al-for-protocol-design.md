...
- **BitSerializer** [📁](./BitSerializer) [🌐](https://github.com/GerHobbelt/BitSerializer) -- C++/17 common interface with serialization style similar to the Boost library; allows easy switching between formats JSON, XML, YAML, CSV and MsgPack; handles different data types, including `std::chrono`, Unicode strings, and data integrity through required/optional constraints; compile-time validation of format rules (e.g. JSON allows primitives as roots, while CSV only allows arrays); loads named fields in any order with conditional logic to preserve model compatibility; seamless handling of optional and required fields, bypassing the need to use `std::optional`; support serialization to memory, streams and files; full Unicode support with automatic detection and transcoding (except YAML).
